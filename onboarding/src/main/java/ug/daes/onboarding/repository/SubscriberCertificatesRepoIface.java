...
import ug.daes.onboarding.model.OnbSubscriberCertificate;

import java.util.List;

@Repository
public interface SubscriberCertificatesRepoIface extends JpaRepository<OnbSubscriberCertificate, String> {
...
//	String getSubscriberCertificateStatus(String uid, String type, String status); // status param retained for signature match (not used)
	...
	@Query("SELECT s.certificateStatus FROM OnbSubscriberCertificate s " +
			"WHERE s.createdDate = (" +
			"SELECT MAX(sc.createdDate) FROM OnbSubscriberCertificate sc " +
	...
	@Query("SELECT s FROM OnbSubscriberCertificate s WHERE s.subscriberUid = ?1 ORDER BY s.createdDate DESC")
	List<OnbSubscriberCertificate> findBySubscriberUniqueId(String subscriberUid);
